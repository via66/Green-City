...
from GreenCity.models import Feature, Park, GreenCityProject, ElectricVehicleChargingStation, BikeRack, \
    CommunityFoodMarket, CommunityGarden


class ParkTestCase(TestCase):
    def setUp(self):
        super(ParkTestCase, self).setUp()
        self.newPark = Park.objects.create(name="ParkName", longitude=12.1234531, latitude=12.1234531,
                                           streetNumber="123", streetName="Streen Name Blablabla", hectare=33.2,
                                           neighbourhoodName="Neighbourhood Name", neighbourhoodURL="", washrooms=True)
...
class BikeRackTestCase(TestCase):
    def setUp(self):
        ...
        self.newBikeRack = BikeRack.objects.create(name=completeAddress,
                                                   longitude=getGeoLocation(completeAddress, "longitude"),
                                                   latitude=getGeoLocation(completeAddress, "latitude"),
                                                   streetNumber=streetNumber, streetName=streetName, numberOfRacks=4)
